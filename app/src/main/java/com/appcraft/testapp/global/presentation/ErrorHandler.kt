...
class ErrorHandler(var resources: Resources, private val router: AppRouter) {
    fun proceed(error: Throwable, allowFullscreenStubs: Boolean = true, callback: (String) -> Unit) {
        ...
        when {
            ...
            // }
            // (error is HttpException) && (error.code() == PROFILE_NOT_FILLED) -> {
            //     router.newRootFlow(Screens.ProfileEditRegistrationFlow(false))
            // }
            else -> {
